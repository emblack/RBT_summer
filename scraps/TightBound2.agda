...
module TightBound2 where
  ...
  module RBT (Key : Set) (compare : Key -> Key -> Order) (Value : Set) where 
    ...
    promote-br-bbrbt :  ∀ {c m n lc rc}
             → RotateType c m (S n) lc rc
             → BalanceResult n (c +1) 
           → Σ (λ oc → BBRBT m (snd (PresBlack c) oc))
    promote-br-bbrbt Black (BB t) = _ , t
    promote-br-bbrbt Red (B t) = _ , nv t 

    rotate-Node : ∀ {c m n lc lc' rc} 
                  (t : RotateType c m n lc rc)
                  (eq : lc == (↓ lc'))
                  (l : RBT' n lc')
                  (x : Key × Value)
                  (r : RBT' n rc)
                → (Σ \ c1 → Σ \ c2 → (RBT' m c1 × (↓ c1 == snd (PresBlack c) c2)))
    rotate-Node Red eq l x r rewrite lemma1 _ _ (! eq) = Red , (↓ Red , (Node Red l x r) , Refl) 
    rotate-Node Black Refl l x r = Black , Black , Node Black l x r , Refl
    
    rotateLeft : ∀ {c m n lc rc}
               → RotateType c m n lc rc
               → BBRBT n lc
               → (Key × Value)
               → RBT' n rc
               → Σ (\ oc → BBRBT m (snd (PresBlack c) oc))
    -- interesting case 1
    rotateLeft {c = rootc} t (v a) x (Node Black b y c) = promote-br-bbrbt t (balanceLeft {c = rootc +1} (RightARBT-RedNode a x b) y c)
    -- interesting case 2
    rotateLeft Black (v a) x (Node Red (Node Black b y c) z d) with (balanceLeft {c = Black} (RightARBT-RedNode a x b) y c)
    ...                                                           | B l' = _ , nv (Node Black l' z d)
    -- keep it the same
    rotateLeft t (nv a) z d with rotate-Node t Refl a z d 
    ...                        | _ , _ , node , eq = _ , transport (BBRBT _) eq (nv node)
    rotateLeft _ _ _ _ = {!!}
